...
{
    public class LineasFinitas: ProcessLog
    {
        public void Calculate(string px, string qx, string rx, double a, double b, double alpha, double beta, int N)
        {
            if (N < 2)
                return;
            ...
            A[0] = (2 + (h * h * Fn(qx, x)));
            B[0] = (-1 + ((h / 2) * Fn(px, x)));
            D[0] = (-h * h * Fn(rx, x) + (1 + (h / 2) * Fn(px, x)) * alpha);
            ...
            A[N - 1] = (2 + h * h * Fn(qx, x));
            C[N - 1] = (-1 - (h / 2) * Fn(px, x));
            D[N - 1] = (-h * h * Fn(rx, x) + (1 - (h / 2) * Fn(px, x)) * beta);
